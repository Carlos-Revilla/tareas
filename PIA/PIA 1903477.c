...
void saveData_at_file(const char* _namefile, alumno* alumnos, calificacionP* calificaciones, grupo g, const size_t tamA, const size_t tamC) {
	...
	size_t i = 0, j = 0;
	size_t len = 0;
	fprintf(stream,"Matricula ; Parcial 1 ; Parcial 2 ; Parcial 3 ; Parcial 4 ; Parcial 5 ; Parcial 6 ; Parcial 7 ; Promedio Calif Parciales ; Numero de Parciales no Aprobados ; \n");
	for(i = 0; i < tamA; ++i) {
		fprintf(stream,"`");
...
int main() {
	...
	load_data("calificaciones_pia_pe.csv",alumnos);
	...
	saveData_at_file("calificaciones_acomodadas.csv",alumnos,calificacionesP,g,TAM,7);
